...
SELECT rating, COUNT(rating), AVG(length) as duration_mean
FROM film
GROUP BY rating;

SELECT rating, COUNT(rating), FLOOR(((length)/ 60)+((length) % 60)) AS duration
FROM film
GROUP BY rating;

SELECT rating, COUNT(rating), (AVG(length)/60) AS duration
FROM film
GROUP BY rating;

-- 7. Which kind of movies (rating) have a mean duration of more than two hours?
SELECT rating, COUNT(rating), ROUND((AVG(length)/60),2) AS duration
FROM film
GROUP BY rating
HAVING duration > 2;
